...
class TWITCHPRESS_All_API {
    ...
    */
    ...
    public function __construct( $service = 'streamlabs', $profile = 'streamlabsdefault' ){
    ...
    */
    public function set_application_credentials() {
        ...
        $this->allapi_subject_id = get_option( 'allapi_' . $this->allapi_service . '_' . $this->allapi_profile . '_subject_id' );
        $this->allapi_app_key    = get_option( 'allapi_' . $this->allapi_service . '_' . $this->allapi_profile . '_key' );
        $this->allapi_app_secret = get_option( 'allapi_' . $this->allapi_service . '_' . $this->allapi_profile . '_secret' );
        $this->allapi_app_uri    = get_option( 'allapi_' . $this->allapi_service . '_' . $this->allapi_profile . '_uri' );
        $this->allapi_app_code   = get_option( 'allapi_' . $this->allapi_service . '_' . $this->allapi_profile . '_code' );
        $this->allapi_app_token  = get_option( 'allapi_' . $this->allapi_service . '_' . $this->allapi_profile . '_token' );   
    ...
    */
    public function is_app_set() {
        ...
        if( !$this->allapi_app_uri ) {
            $missing[] = __( 'Application URI', 'twitchpress' );        
        }    
        
        if( !$this->allapi_app_key ) {
            $missing[] = __( 'Application Key', 'twitchpress' );        
        }    
        
        if( !$this->allapi_app_secret ) {
            $missing[] = __( 'Application Secret', 'twitchpress' );        
        }    
        
        if( !$this->allapi_app_code ) {
            $missing[] = __( 'Application Code', 'twitchpress' );        
        }    
        
        if( !$this->allapi_app_token ) {
            $missing[] = __( 'Application Token', 'twitchpress' );        
        }       
    ...
    */
    public static function administrator_main_account_listener() {
        ...
        if( !$test_result )
        {
            TwitchPress_Admin_Notices::add_custom_notice( 'allapi_listener_test_failed', __( '<strong>Final Test Failed:</strong> The administrator account listener has passed validation but failed the first attempt to request data from the services server.', 'twitchpress' ) );      
            ...
            $bugnet->trace( 'allapi_oauth2mainaccount',
                ...
                __FILE__,
                true,
                __( 'All-API Account Listener: the giving subject cannot be confirmed as the server response indicates a failure.', 'twitchpress' )
            );
        ...
        $bugnet->trace( 'allapi_oauth2mainaccount',
            ...
            __FILE__,
            true,
            __( 'All-API Admin Account Listener: Pass, forwarding user to: ' . $transient_state['redirectto'], 'twitchpress' )
        );
    ...
    public function app_credentails_test( $service, $service_calls_object ) {
        ...
        switch ( $service ) {
           case 'twitch':
                ...
                $user_objects = $service_calls_object->get_users( $service_calls_object->twitch_default_channel );
        
                if( !isset( $user_objects['users'][0]['_id'] ) ) {
                    TwitchPress_Admin_Notices::add_custom_notice( 'allapi_listenersubjectdoesnotexist', __( '<strong>Main Channel Not Found:</strong> TwitchPress could not find the giving channel name. Please check the spelling of your channel and the status of Twitch. If your entered channel name is correct and Twitch is online, please report this notice.', 'twitchpress' ) );      
                ...
                // Backwards compatability for TwitchPress Kraken and probably Helix. 
