<?php
if ( ! class_exists( 'TwitchPress_Login_Helix' ) ) :

    class TwitchPress_Login_Helix {
        /**
         * @var Singleton
         */
        ...
        */
        ...
        function enqueue_public_css() {
            ...
            wp_register_style( 'twitchpress-login-extension-public-styles', TwitchPress_Login_Helix::plugin_url() . '/assets/css/public.css' );            
            wp_enqueue_style( 'twitchpress-login-extension-public-styles', TwitchPress_Login_Helix::plugin_url() . '/assets/css/public.css' ); 
        }
        ...
        */
        public function twitchpress_login_styles() {
            ...
            wp_register_style( 'twitchpress_login_extension_styles', TwitchPress_Login_Helix::plugin_url() . '/assets/css/public.css' );
        ...
        */
        public function twitch_login_public_listener() {              
            ...
            if( $transient_state['view'] == 'post' ) { 
                if( !$transient_state['loginpageid'] || !is_numeric( $transient_state['loginpageid'] ) ) { 
                    // Log
                    $bugnet->log_error( __FUNCTION__, sprintf( __( 'Login listener ended - missing login page ID.', 'twitchpress' ), $_GET['error'] ), array(), true );
                    // Die
        ...
        private function process_login_using_twitch() {  
            global $bugnet;
            ...
            $token_array = $twitch_api->request_user_access_token( $_GET['code'], __FUNCTION__ );   

            // Confirm token was returned.  
            if( !twitchpress_was_valid_token_returned( $token_array ) ) {   
                $this->loginerror( __( 'Token Invalid'), 
                                   __( 'Login via Twitch (Helix) failed because the token is invalid.', 'twitchpress-login' ),
                                   null );                                  
                return;                         
            }
            
            // Get the visitors Twitch details.
            $twitch_user = $twitch_api->getUserObject_Authd( $token_array['access_token'], $_GET['code'] );
            ...
            $twitch_user['_id']               = sanitize_key( $twitch_user['_id'] ); // 44322889,
            $twitch_user['bio']               = sanitize_text_field( $twitch_user['bio'] ); // "Just a gamer playing games and chatting. :)",
            $twitch_user['created_at']        = sanitize_text_field( $twitch_user['created_at'] ); // "2013-06-03T19:12:02Z",
            ...
            $twitch_user['email_verified']    = sanitize_text_field( $twitch_user['email_verified'] ); // true,
            $twitch_user['logo']              = esc_url_raw( $twitch_user['logo'] ) ; // "https://static-cdn.jtvnw.net/jtv_user_pictures/dallas-profile_image-1a2c906ee2c35f12-300x300.png",
            $twitch_user['name']              = sanitize_user( $twitch_user['name'], true ); // "dallas",
            $twitch_user['partnered']         = sanitize_text_field( $twitch_user['partnered'] ); // false,
            $twitch_user['updated_at']        = sanitize_text_field( $twitch_user['updated_at'] ); // "2016-12-14T01:01:44Z"
            ...
            {     
                ...
                {   
                    ...
                    update_user_meta( $wp_user_id, 'twitchpress_twitch_created_at', $twitch_user['created_at'] );
                    update_user_meta( $wp_user_id, 'twitchpress_twitch_display_name', $twitch_user['display_name'] );
                    update_user_meta( $wp_user_id, 'twitchpress_twitch_email_verified', $twitch_user['email_verified'] );
                    update_user_meta( $wp_user_id, 'twitchpress_twitch_name', $twitch_user['name'] );
                    update_user_meta( $wp_user_id, 'twitchpress_twitch_partnered', $twitch_user['partnered'] );
                    update_user_meta( $wp_user_id, 'twitchpress_twitch_updated_at', $twitch_user['updated_at'] );
            ...
            {      
                ...
                update_user_meta( $wp_user->data->ID, 'twitchpress_twitch_created_at', $twitch_user['created_at'] );
                update_user_meta( $wp_user->data->ID, 'twitchpress_twitch_display_name', $twitch_user['display_name'] );
                update_user_meta( $wp_user->data->ID, 'twitchpress_twitch_email_verified', $twitch_user['email_verified'] );
                update_user_meta( $wp_user->data->ID, 'twitchpress_twitch_name', $twitch_user['name'] );
                update_user_meta( $wp_user->data->ID, 'twitchpress_twitch_partnered', $twitch_user['partnered'] );
                update_user_meta( $wp_user->data->ID, 'twitchpress_twitch_updated_at', $twitch_user['updated_at'] );
            ...
            do_action( 'twitchpress_login_inserted_new_user', $wp_user_id );
            ...
            self::authenticate_login_by_twitch( $wp_user_id, $twitch_user['name'], $state_code );
        }               
...
if( !function_exists( 'TwitchPress_UM_Ext' ) ) {
    ...
    function TwitchPress_Login_Ext() {        
        return TwitchPress_Login_Helix::instance();
    }
...
register_activation_hook( __FILE__, array( 'TwitchPress_Login_Helix', 'install' ) );
register_deactivation_hook( __FILE__, array( 'TwitchPress_Login_Helix', 'deactivate' ) );
